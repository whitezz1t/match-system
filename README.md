...
**A:** 这是一个 Docker 路径映射问题。
**解决方案**：本项目使用了 `WebConfig` 自动识别环境，在 Docker 中会自动映射 `file:/app/videos/`。请确保数据库 `video_file_path` 字段存储的是**纯文件名** (如 `round_1.mp4`) 而不是绝对路径。

**Q3: 数据库修改了 init.sql 但不生效？**
...
**Ping Pong Match System** | Made with ❤️ by whitezz1t
